...
def create_labels(root_node, labels_root):
    ...
    for frame in frames:
        ...
        img_root = './train/images/' if 'train' in labels_root else './valid/images/'
        ...
        if 'MVI' in video_name:
            ...
            for i, img_name in enumerate(os.listdir(os.path.join(opt.img_train, video_name))):
                if ((i % stride) == 0) & ('.jpg' in img_name):
                    ...
                    new_path = os.path.join('./train/images/', video_name + '_' + img_name)
        ...
        if 'MVI' in video_name:
            ...
            for i, img_name in enumerate(os.listdir(os.path.join(opt.img_val, video_name))):
                if ((i % stride) == 0) & ('.jpg' in img_name):
                    ...
                    new_path = os.path.join('./valid/images/', video_name + '_' + img_name)
                    shutil.copyfile(ori_path, new_path)
...
def gen_yolo_dataset(opt):
    ...
    for video_xml in tqdm(os.listdir(opt.lbl_train)):
        if '.xml' in video_xml:
            create_labels(parse(os.path.join(opt.lbl_train, video_xml)).documentElement, './train/labels/')
    ...
    for video_xml in tqdm(os.listdir(opt.lbl_val)):
        if '.xml' in video_xml:
            create_labels(parse(os.path.join(opt.lbl_val, video_xml)).documentElement, './valid/labels/')
